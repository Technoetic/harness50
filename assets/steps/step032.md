...
1. **파일 목록 생성**: `step030_레이아웃설계_chunk*.md`와 `step030_전체설계_chunk*.md`에서 클래스/함수 추출, 파일 경로 결정, 신규/수정 구분
...
필수 요구와 현재 검증 증거가 모두 PASS일 때만 이 지침을 완료하고 자동으로 step033.md를 읽고 수행한다. 사용자 확인을 기다리지 않는다.
