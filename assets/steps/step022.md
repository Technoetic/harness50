...
Step 20에서 선정된 URL을 브라우저 자동화 도구(docs/BROWSER-TOOLS.md: Aside CLI 또는 Playwright)로 방문하여 스크린샷과 텍스트를 수집한다.
...
1. 브라우저 자동화 수집 절차 작성·실행 (뷰포트·스크린샷·텍스트 추출은 docs/BROWSER-TOOLS.md 절차표)
2. 뷰포트별 스크린샷 촬영
3. 텍스트 콘텐츠 추출
4. 원본 데이터 파일 저장
...
- 각 참고 사이트의 **모든 페이지**를 브라우저 자동화 도구로 방문하여 뷰포트별 스크린샷을 촬영한다.
- **페이지 수집 규칙:**
...
- **뷰포트 결정 규칙:**
  ...
  - 반응형 프로젝트: 데스크톱 (1920×1080) + 태블릿 (768×1024) + 모바일 (390×844) (Aside 백엔드는 탭 고정 1440×900 — 실제 캡처 크기를 manifest에 기록)
