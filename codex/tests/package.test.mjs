...
test("Codex manifest isolates Codex skills and hooks", async () => {
  ...
  assert.match(manifest.version, /^2\.5\.0(?:\+codex\.[a-z0-9-]+)?$/);
...
test("Claude, Codex, and marketplace release versions are synchronized with optional Codex build metadata", async () => {
  ...
  if (codex.version.includes("+")) assert.match(codex.version, /^2\.5\.0\+codex\.[a-z0-9-]+$/);
  ...
  assert.equal(marketplace.metadata.version, "2.5.0");
  assert.equal(entry?.source, "./");
  assert.equal(entry?.version, "2.5.0");
