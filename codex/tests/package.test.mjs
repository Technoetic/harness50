...
test("Codex manifest isolates Codex skills and hooks", async () => {
  ...
  assert.match(manifest.version, /^2\.4\.3(?:\+codex\.[a-z0-9-]+)?$/);
...
test("Claude, Codex, and marketplace release versions are synchronized with optional Codex build metadata", async () => {
  ...
  assert.equal(claude.version, "2.4.3");
  assert.equal(codex.name, "harness50");
  assert.equal(codex.version.split("+")[0], claude.version);
  if (codex.version.includes("+")) assert.match(codex.version, /^2\.4\.3\+codex\.[a-z0-9-]+$/);
  ...
  assert.equal(marketplace.metadata.version, "2.4.3");
  assert.equal(entry?.source, "./");
  assert.equal(entry?.version, "2.4.3");
