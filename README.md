...
개발 중인 다음 버전은 Claude·Codex의 제품 QA에 [실패·검증 인계 보고서](docs/QA-REPORTS.md)를 연결합니다. 검증 전에 지정 파일과 필수 검사를 snapshot으로 기록하고, 실제 결과를 저장한 뒤 다음 시도에서 확인합니다. 파일이나 증거가 바뀌면 이전 성공을 현재 보존 근거로 쓰지 않으며, 기존 완료 게이트는 계속 적용됩니다.
...
**44단계는 ‘클라이언트 사이드 라우팅’입니다.** 30단계에서 설계하고 37단계에서 구현한
화면 주소를 JS·CSS 구조 정리 뒤 다시 통합 점검합니다. 같은 HTML의 화면별 영역을
URL에 따라 표시할 수 있으며, HTML 컴포넌트 추출은 필수가 아닙니다. 전체 흐름은 50단계입니다.
